...
data_dir = pathlib.Path(r'D:\Code\dataset\recognization')
...
def create_model(hp):
    model = Sequential([
        ...
        #卷积神经网络
        layers.Conv2D(hp.Choice('num_filters_layer0',values=[16,64],default=16),3,padding='same',activation='relu'),
        layers.MaxPooling2D(),
        layers.Conv2D(hp.Choice('num_filters_layer1',values=[16,64],default=16),3,padding='same',activation='relu'),
        layers.MaxPooling2D(),
        layers.Conv2D(hp.Choice('num_filters_layer2',values=[16,64],default=16),3,padding='same',activation='relu'),
        layers.MaxPooling2D(),
        #添加Dropout
        layers.Dropout(0.2),
        #常规三层
        layers.Flatten(),
        layers.Dense(hp.Int("hiden_units",128,512,step=32),activation='relu'),
...
tuner = Hyperband(
    create_model,#告诉他用哪个函数来生成模型
    objective='val_accuracy',
    max_epochs=15,#猜15次达到
    #调参的时候会将数据保存到本地,需要指定路径
    directory='recognization',
    hyperparameters=hp,#告诉他使用这个变量
    project_name='face_recognization_project' #存储的时候随便取的名字
)
#搜一下参数
tuner.search(train_ds,validation_data=val_ds)#search里面的内容要和fit里面内容相同
